...
public class Project {
	...
	//check qualifications. or do that up stream? might need to do a check for this in the constructor too.
	/* Check for boundary conditions on the qualification set as well as the 
	requirements on the String reference (not null). These qualifications must be from the 
	...
	public void addQualification(Qualification qualification) {
		...
		if(this.status == ProjectStatus.ACTIVE && !this.getMissingQualifications().isEmpty()){ // if no longer met, should either be Planned or 
			//status = ProjectStatus.PLANNED;
